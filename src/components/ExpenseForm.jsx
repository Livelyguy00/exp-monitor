  ...
  return (
    <form id='expenseForm' onSubmit={() => onSubmitEvent({date, category, description, cost})}>
      <div className='form-row align-items-center mx-4 py-5'>
        <FormInput name='date' defaultValue={date} type='date' label='Data' onChange={value => setDate(value)} colSize={2}  />
        <FormInput name='category' defaultValue={category} type='text' label='Kategoria' onChange={value => setCategory(value)} colSize={3} />
        <FormInput name='description' defaultValue={description} type='text' label='Opis' onChange={value => setDescription(value)} colSize={4} />
        <FormInput name='value' defaultValue={cost} type='number' label='Koszt' onChange={value => setCost(value)} colSize={2} />
