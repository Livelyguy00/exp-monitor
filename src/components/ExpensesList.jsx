  ...
  const renderExpense = (id, {date, category, description, cost}) => {
    return(
      <CSSTransition
        key={id}
        timeout={300}
        classNames='expense-animation-'
      >
        <Expense id={parseInt(id)} date={date} category={category} cost={cost} description={description} onSaveExpense={editExpense(id)} onDeleteClick={removeExpense}/>
      </CSSTransition>
    )
  }
  ...
  return (
    <div className='p-5 position-relative expensesList'>
      <div className='clearExpenses position-absolute'>
        ...
        <span className='clearExpenses__label'>Wyczyść wydatki</span>
      </div>
      <table className='table'>
        <thead>
          <tr className='row'>
            ...
            <th className='col-2 text-center'>Kategoria</th>
            <th className='col-4 text-center'>Opis</th>
            <th className='col-1 text-center'>Kwota</th>
          </tr>
        </thead>
      </table>
